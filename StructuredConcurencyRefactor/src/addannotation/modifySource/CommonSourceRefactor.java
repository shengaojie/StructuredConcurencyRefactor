...
public class CommonSourceRefactor extends SourceRefactor {

	public CommonRefactorNode refactorNode;
	...
	public CommonSourceRefactor(CommonRefactorNode refactorNode) {
		super(VerifyIfMatched.astRoot, refactorNode.executorMap, refactorNode.methodDeclaration);
		this.refactorNode = refactorNode;
		
		this.futureMap = refactorNode.futureMap;
		this.getMap = refactorNode.getMap;
		this.invokeMap = refactorNode.invokeMap;
		this.stmtbeSubAndGet = refactorNode.stmtbeSubAndGet;
		this.stmtbeGetAndInvoke = refactorNode.stmtbeGetAndInvoke;
		
	
	}

	
	public void CommonSourceRefactor() {
	...
	public void tryRefactor() {
		System.out.println("tryRefactor enter") ;
		AST ast = root.getAST();
		...
		addjoin(ast, tryBlock);
		
		addThrowIfFaied(ast, tryBlock);
		...
		System.out.println("tryResource10 is :" + tryResource) ;
	...
	private void addResource(TryStatement tryResource,Block tryBlock,AST ast) {
		...
		methodDeclaration.getBody().statements().add(tryPosition,tryResource);
	...
	private void addFutures(Block tryBlock,AST ast) {
		...
		//先将所有的future加入到try的body中
		for (Integer key : futureMap.keySet()) {
		    VariableDeclarationStatement future = (VariableDeclarationStatement)futureMap.get(key);
	...
	public void addjoin(AST ast,Block tryBlock) {
	...
	public void addThrowIfFaied(AST ast,Block tryBlock) {
	...
	private void addGets(Block tryBlock,AST ast) {
		...
		for(Integer key : getMap.keySet()) {
			VariableDeclarationStatement getStatement = (VariableDeclarationStatement)getMap.get(key);
